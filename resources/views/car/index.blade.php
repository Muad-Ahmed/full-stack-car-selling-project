                            ...
                            <tbody>
                                @forelse($cars as $car)
                                    <tr>
                                        ...
                                        <td data-label="Date" class="hide-on-mobile">{{ $car->getCreateDate() }}</td>
                                        <td data-label="Published" class="hide-on-mobile">
                                            <span
                                                class="badge {{ $car->published_at ? 'badge-success' : 'badge-warning' }}">
                                                {{ $car->published_at ? 'Yes' : 'No' }}
                                            </span>
                                        </td>
                                        <td data-label="Actions" class="actions-cell">
                                            <div class="actions-wrapper">
                                                <a href="{{ route('car.edit', $car) }}" class="btn-action btn-edit-icon"
                                                    title="Edit">
                                                ...
                                                <a href="{{ route('car.images', $car) }}"
                                                    class="btn-action btn-images-icon" title="Images">
                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none"
                                                        viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                                        <path stroke-linecap="round" stroke-linejoin="round"
                                ...
                                @empty
                                    <tr>
                                        <td colspan="5" class="text-center empty-state">
                                            You don't have any cars yet. <a href="{{ route('car.create') }}"
                                                class="link-primary">Add new car</a>
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
